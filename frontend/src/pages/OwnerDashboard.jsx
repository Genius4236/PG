...
const OwnerDashboard = () => {
	...
	// Controlled values for custom Selects used in the Add form
	...
	// Robust FormData builder — uses controlled selects + specific inputs by name
	const handleAddPG = async (e) => {
		...
		try {
			...
			// Inputs must have name attributes (see form JSX below)
			formData.append('name', e.target.name.value);
			formData.append('city', selectedCity || e.target.city?.value || '');
			formData.append('locality', e.target.locality.value);
			formData.append('address', e.target.address.value);
			formData.append('gender', selectedGender || e.target.gender?.value || '');
			formData.append('description', e.target.description.value);

			// Build sharing_types matching backend enum casing
			const sharingTypes = [
				{ type: 'Single', price: parseInt(e.target.singlePrice.value || 0, 10), available: true },
				{ type: 'Double', price: parseInt(e.target.doublePrice.value || 0, 10), available: true }
			];
			if (e.target.triplePrice && e.target.triplePrice.value) {
				sharingTypes.push({ type: 'Triple', price: parseInt(e.target.triplePrice.value, 10), available: true });
			}
			formData.append('sharing_types', JSON.stringify(sharingTypes));
			...
			toast({ title: 'PG Added Successfully', description: 'Your PG has been listed on the platform' });
			...
			console.error(err);
			toast({ title: 'Error', description: 'Failed to add PG', variant: 'destructive' });
		} finally {
			setSubmitting(false);
		}
	};
	...
	return (
		<div className="min-h-screen bg-gradient-to-b from-slate-50 to-white py-8">
			<div className="container mx-auto px-4">
				{/* Header */}
				<div className="bg-gradient-to-br from-white to-slate-50 rounded-lg shadow-2xl border border-slate-200 p-6 mb-6">
					<div className="flex justify-between items-center">
						<div>
							<h1 className="text-3xl font-bold text-teal-700">PG Owner Dashboard</h1>
						...
						<div className="flex gap-3">
							<Dialog open={isAddPGOpen} onOpenChange={(open) => {
								...
								<DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl border border-slate-200 backdrop-blur-sm">
									<DialogHeader>
									...
									<form onSubmit={handleAddPG} className="space-y-4">
										<div className="grid md:grid-cols-2 gap-4">
											<div className="space-y-2">
												...
												<Input id="pgName" name="name" placeholder="Green Valley PG" required />
											</div>
											<div className="space-y-2">
												...
												{/* Hidden input to ensure FormData includes city value */}
												<input type="hidden" name="city" value={selectedCity || ''} />
											</div>
										</div>
										<div className="space-y-2">
											<Label htmlFor="locality">Locality</Label>
											<Input id="locality" name="locality" placeholder="Koramangala" required />
										</div>
										<div className="space-y-2">
											<Label htmlFor="address">Full Address</Label>
											<Textarea id="address" name="address" placeholder="Complete address..." required />
										</div>
										<div className="space-y-2">
											<Label>Gender</Label>
											<Select
											...
											{/* Hidden input to ensure FormData includes gender value */}
											<input type="hidden" name="gender" value={selectedGender || ''} />
										</div>
										<div className="space-y-2">
											<Label htmlFor="description">Description</Label>
											<Textarea id="description" name="description" placeholder="Describe your PG..." required />
										</div>
										<div className="grid grid-cols-3 gap-4">
											<div className="space-y-2">
												<Label htmlFor="singlePrice">Single Room Price</Label>
												<Input id="singlePrice" name="singlePrice" type="number" placeholder="12000" required />
											</div>
											<div className="space-y-2">
												<Label htmlFor="doublePrice">Double Room Price</Label>
												<Input id="doublePrice" name="doublePrice" type="number" placeholder="8000" required />
											</div>
											<div className="space-y-2">
												<Label htmlFor="triplePrice">Triple Room Price</Label>
												<Input id="triplePrice" name="triplePrice" type="number" placeholder="6000" />
											</div>
										</div>
										<div className="space-y-2">
										...
										<Button type="submit" className="w-full bg-gradient-to-r from-teal-600 to-teal-700 hover:from-teal-700 hover:to-teal-800 shadow-lg hover:shadow-xl hover:scale-105 transition duration-300" disabled={submitting}>
											{submitting ? (
												<>
							...
							<Button onClick={handleLogout} variant="outline" className="flex items-center gap-2 hover:border-teal-500 hover:text-teal-600 transition">
				...
				<Tabs defaultValue="overview" className="space-y-6">
					...
					<TabsContent value="overview" className="space-y-6">
						...
						<div className="grid md:grid-cols-4 gap-6">
							<Card className="shadow-xl hover:shadow-2xl transition-all duration-300 border border-slate-100 hover:border-teal-200">
								<CardHeader>
								...
								<CardContent>
									<p className="text-4xl font-bold text-teal-700">{ownerPGs.length}</p>
								</CardContent>
							</Card>
							<Card className="shadow-xl hover:shadow-2xl transition-all duration-300 border border-slate-100 hover:border-teal-200">
								<CardHeader>
						...
						<Card className="shadow-xl border border-slate-100">
							...
							<CardContent>
								<div className="space-y-4">
									<div className="flex justify-between items-center p-3 bg-slate-50 rounded-lg hover:bg-slate-100 transition-colors border border-slate-200">
										<div>
					...
					<TabsContent value="properties" className="space-y-6">
						<div className="grid md:grid-cols-2 gap-6">
							{ownerPGs.length === 0 ? (
								<Card className="md:col-span-2 shadow-xl border border-slate-100">
									<CardContent className="p-12 text-center">
										...
										<Button onClick={() => setIsAddPGOpen(true)} className="bg-gradient-to-r from-teal-600 to-teal-700 hover:from-teal-700 hover:to-teal-800 shadow-lg hover:shadow-xl hover:scale-105 transition duration-300">
											Add Your First PG
										</Button>
									</CardContent>
								</Card>
								...
								ownerPGs.map((pg) => (
									<Card key={pg.id} className="shadow-xl hover:shadow-2xl transition-all duration-300 border border-slate-100 hover:border-teal-200 overflow-hidden">
										<CardContent className="p-0">
											<img src={pg.images && pg.images.length > 0 ? pg.images[0] : '/placeholder-pg.jpg'} alt={pg.name} className="w-full h-48 object-cover hover:scale-110 transition-transform duration-300" />
											<div className="p-6">
												<h3 className="text-xl font-bold text-teal-700 mb-2">{pg.name}</h3>
												...
												<div className="flex gap-2">
													<Button variant="outline" className="flex-1 flex items-center gap-2 hover:border-teal-500 hover:text-teal-600 transition" onClick={() => handleEditPG(pg)}>
														<Edit className="h-4 w-4" />
														Edit
													</Button>
													<Button variant="outline" className="flex-1 flex items-center gap-2 text-red-600 hover:text-red-700 hover:border-red-500 transition" onClick={() => handleDeletePG(pg.id)}>
								...
								<div className="space-y-4">
									{bookings.length === 0 ? (
										...
										bookings.map((booking) => (
											<div key={booking.id} className="border border-slate-200 rounded-lg p-4 hover:shadow-lg hover:border-teal-200 transition-all duration-300">
